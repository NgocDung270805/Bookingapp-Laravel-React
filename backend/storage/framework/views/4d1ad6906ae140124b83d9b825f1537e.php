...
<head>
    ...
    <script>
        ...
        // Subscribe to dashboard-stats channel
        ...
        // Lắng nghe tất cả các events để debug
        channel.bind_global((eventName, data) => {
        });

        // Lắng nghe sự kiện stats.updated
        channel.bind('stats.updated', function(data) {

        // Cập nhật số liệu ngay khi nhận được
        ...
        // Đóng script tag và body khi document load xong

        // Update DOM elements
        const newBookings = document.querySelector('[data-stat="new_bookings"]');
        const pendingBookings = document.querySelector('[data-stat="pending_bookings"]');

        if (newBookings && data.new_bookings !== undefined) {
            newBookings.textContent = data.new_bookings;
        }

        if (pendingBookings && data.pending_bookings !== undefined) {
            pendingBookings.textContent = data.pending_bookings;
        }
